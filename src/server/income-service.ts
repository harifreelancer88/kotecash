...
export async function actualIncomeMovements(db:D1Database,userId:number,from:string,to:string){
  return (await db.prepare(`SELECT m.*, c.name category_name, c.type category_type, cc.classification FROM movements m LEFT JOIN categories c ON c.id=m.category_id AND c.user_id=m.user_id LEFT JOIN category_classifications cc ON cc.category_id=m.category_id AND cc.user_id=m.user_id WHERE m.user_id=? AND m.date BETWEEN ? AND ? AND m.src_kind IS NULL AND m.dst_kind IS NOT NULL ORDER BY m.date,m.id`).bind(userId,from,to).all<any>()).results || [];
}
