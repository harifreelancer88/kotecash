...
export async function calculateBalance(c:AppContext, uid:number, walletId:number, asOf:string){
  ...
  const m=(await c.env.DB.prepare(`SELECT * FROM movements WHERE user_id=? AND COALESCE(status,'active')='active' AND date(date)>date(?) AND date(date)<=date(?) AND ((src_kind='wallet' AND src_id=?) OR (dst_kind='wallet' AND dst_id=?)) ORDER BY date,id`).bind(uid,start,asOf,walletId,walletId).all<any>()).results;
  ...
  const pre=await c.env.DB.prepare(`SELECT COUNT(*) count FROM movements WHERE user_id=? AND COALESCE(status,'active')='active' AND ((src_kind='wallet' AND src_id=?) OR (dst_kind='wallet' AND dst_id=?)) AND date(date)<date(?)`).bind(uid,walletId,walletId,start).first<any>();
...
export async function previewReconciliation(c:AppContext, uid:number, rec:any){
  ...
  const mv=(await c.env.DB.prepare(`SELECT * FROM movements WHERE user_id=? AND COALESCE(status,'active')='active' AND date(date)>=date(?) AND date(date)<=date(?) AND ((src_kind='wallet' AND src_id=?) OR (dst_kind='wallet' AND dst_id=?))`).bind(uid,rec.period_start,rec.period_end,rec.wallet_id,rec.wallet_id).all<any>()).results;
