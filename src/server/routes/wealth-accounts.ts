...
import { portfolioValue, type Movement, type Snapshot } from "../formulas";
...
wealthAccounts.get("/", async (c: AppContext) => {
  ...
  const snaps = await c.env.DB.prepare("SELECT entity_kind, entity_id, amount, recorded_at FROM balance_history WHERE user_id=? AND entity_kind='portfolio'").bind(uid).all<Snapshot>();
  const mv = await c.env.DB.prepare("SELECT src_kind, src_id, dst_kind, dst_id, amount, date FROM movements WHERE user_id=?").bind(uid).all<Movement>();
  return c.json(res.results.map((p: any) => ({ ...p, currentValue: portfolioValue(p.id, snaps.results, mv.results) })));
});
