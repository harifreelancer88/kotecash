...
async function analyze(c: AppContext, uid: number, clientId: string, tx: Tx) {
  ...
  const smsKey = clean(tx.sms_fingerprint, 160) || clean(tx.raw_sms_hash, 160);
  const financial = await hash([tx.transaction_date, clean(tx.transaction_time, 16) ?? "", amount, tx.direction, tx.wallet_id ?? "", tx.destination_wallet_id ?? "", clean(tx.reference_number, 80) ?? "", clean(tx.merchant, 120) ?? ""].join("|"));
  ...
  return { issues, unsupported, source, amount, financial, request, smsKey, duplicate_status, existing, movement: issues.length ? null : movementFor(tx) };
}
