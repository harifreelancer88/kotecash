...
var ledgerState = { selectionMode: false, filtersOpen: false, selected: new Set(), deleting: false, filters: { q: "", cat: "", type: "", dateMode: "all", date: "", from: "", to: "", month: "" } };

document.addEventListener("click", function(e) {
  var btn = e.target && e.target.closest ? e.target.closest("[data-ledger-action]") : null;
  if (!btn) return;
  e.preventDefault();
  var action = btn.getAttribute("data-ledger-action");
  if (action === "toggle-filters") toggleLedgerFilters();
  if (action === "toggle-bulk") setLedgerSelectionMode(!ledgerState.selectionMode);
  if (action === "add-transaction" && !ledgerState.selectionMode) showAddTransaction();
});
...
function renderDashboard() {
  ...
  var attention = (al.initial_items||[]).slice(0,5).map(function(a){ var sev=String(a.severity||'info').toLowerCase(); var label=sev==='critical'||sev==='high'?'High':sev==='medium'||sev==='warning'?'Medium':sev==='low'?'Low':'Info'; return '<div class="card-row dashboard-card items-start"><span class="severity-badge severity-'+label.toLowerCase()+' flex-shrink-0">'+label+'</span><div class="min-w-0 flex-1"><div class="text-sm font-bold">'+esc(a.title)+'</div><div class="text-xs" style="color:var(--c-sub);">'+esc(a.explanation)+'</div></div><a class="btn btn-xs min-h-[44px]" href="'+esc(a.destination_path||'/')+'" data-page="'+(((a.destination_path||'').match(/page=([^&]+)/)||[])[1]||'dashboard')+'">Open</a></div>'; }).join('') || '<div class="card dashboard-card p-4 text-sm" style="color:var(--c-sub);">No attention items right now.</div>';
...
function ledgerFilterGroup(id, label, control) { return '<div class="ledger-filter-group"><label for="'+id+'" class="ledger-filter-label">'+label+'</label>'+control+'</div>'; }
function ledgerDateLabel(f) {
  if (f.dateMode === "specific" && f.date) return fmtLedgerDate(f.date);
  if (f.dateMode === "range" && (f.from || f.to)) return (f.from ? fmtLedgerDate(f.from) : "Start") + " to " + (f.to ? fmtLedgerDate(f.to) : "Today");
  if (f.dateMode === "month" && f.month) return f.month;
  return f.dateMode !== "all" ? "Date set" : "";
}
function ledgerFilterChips() {
  var f = ledgerState.filters, chips = [];
  if (f.q) chips.push(['q', 'Search: '+f.q]);
  if (f.cat) chips.push(['cat', f.cat]);
  if (f.type) chips.push(['type', f.type === 'income' ? 'Income' : 'Expense']);
  var dl = ledgerDateLabel(f); if (dl) chips.push(['date', dl]);
  if (!chips.length) return '';
  return '<div class="ledger-filter-chips" aria-label="Active ledger filters">'+chips.map(function(c){return '<button type="button" class="ledger-chip" onclick="clearLedgerFilter(&#39;'+c[0]+'&#39;)">'+esc(c[1])+' <span aria-hidden="true">x</span></button>';}).join('')+'<button type="button" class="ledger-chip ledger-chip-clear" onclick="clearLedgerFilters()">Clear</button></div>';
}
function renderLedgerDateFields(f) {
  if (f.dateMode === "specific") return ledgerFilterGroup('ledDate', 'Transaction date', '<input id="ledDate" type="date" value="'+esc(f.date)+'" onchange="filterLedger()">');
  if (f.dateMode === "range") return ledgerFilterGroup('ledFrom', 'From date', '<input id="ledFrom" type="date" value="'+esc(f.from)+'" onchange="filterLedger()">') + ledgerFilterGroup('ledTo', 'To date', '<input id="ledTo" type="date" value="'+esc(f.to)+'" onchange="filterLedger()">');
  if (f.dateMode === "month") return ledgerFilterGroup('ledMonth', 'Month', '<input id="ledMonth" type="month" value="'+esc(f.month)+'" onchange="filterLedger()">');
  return '';
}
function clearLedgerDateValuesForMode(f) { if (f.dateMode !== "specific") f.date=''; if (f.dateMode !== "range") { f.from=''; f.to=''; } if (f.dateMode !== "month") f.month=''; }
function ledgerBulkHTML(){ return ledgerState.selectionMode?'<div class="ledger-bulk"><b>'+ledgerState.selected.size+' selected</b><button onclick="selectAllVisibleLedger()">Select all visible</button><button onclick="clearLedgerSelection()">Clear</button><button class="danger" '+(!ledgerState.selected.size||ledgerState.deleting?'disabled':'')+' onclick="bulkDeleteTransactions()">Delete selected</button><button onclick="setLedgerSelectionMode(false)">Cancel</button></div>':''; }
function renderLedgerControls(){ var cats=M.expenseCats.concat(M.incomeCats), catOpts=cats.map(function(c){return '<option '+(ledgerState.filters.cat===c?'selected':'')+'>'+esc(c)+'</option>';}).join(''); var f=ledgerState.filters; clearLedgerDateValuesForMode(f); var dateOpts='<option value="all" '+(f.dateMode==='all'?'selected':'')+'>All dates</option><option value="specific" '+(f.dateMode==='specific'?'selected':'')+'>Specific date</option><option value="range" '+(f.dateMode==='range'?'selected':'')+'>Date range</option><option value="month" '+(f.dateMode==='month'?'selected':'')+'>Month</option>'; var count=activeLedgerFilterCount(); var bulkLabel=ledgerState.selectionMode?'Cancel':'Bulk select'; return '<div class="ledger-toolbar"><input id="ledSearch" placeholder="Search notes, payee, amount..." class="input input-sm bg-white text-sm ledger-search" value="'+esc(f.q)+'" oninput="filterLedger()"><button class="ledger-filter-toggle" aria-expanded="'+(ledgerState.filtersOpen?'true':'false')+'" data-ledger-action="toggle-filters"><i data-lucide="sliders-horizontal" class="w-4 h-4"></i> Filters'+(count? ' ('+count+')':'')+'</button><button class="ledger-filter-toggle ledger-bulk-toggle" data-ledger-action="toggle-bulk">'+bulkLabel+'</button><button class="btn-primary ledger-add" '+(ledgerState.selectionMode?'disabled':'')+' data-ledger-action="add-transaction"><i data-lucide="plus" class="w-4 h-4"></i> Add transaction</button></div>'+ledgerFilterChips()+'<div id="ledgerFilters" class="ledger-filters '+(ledgerState.filtersOpen?'open':'')+'">'+ledgerFilterGroup('ledCat','Category','<select id="ledCat" onchange="filterLedger()"><option value="">All categories</option>'+catOpts+'</select>')+ledgerFilterGroup('ledType','Transaction type','<select id="ledType" onchange="filterLedger()"><option value="">All types</option><option '+(f.type==='income'?'selected':'')+'>Income</option><option '+(f.type==='expense'?'selected':'')+'>Expense</option></select>')+ledgerFilterGroup('ledDateMode','Date filter','<select id="ledDateMode" onchange="filterLedger()">'+dateOpts+'</select>')+renderLedgerDateFields(f)+'<button type="button" onclick="clearLedgerFilters()">Reset filters</button></div><div id="ledgerBulk">'+ledgerBulkHTML()+'</div>'; }
...
function refreshLedgerRows(){ var rows=getFilteredLedgerRows(); var el=document.getElementById('ledgerRows'); if(el) el.innerHTML=rows.map(ledgerRowHTML).join('')||'<div class="card-row"><span class="text-xs" style="color:var(--c-sub);">No transactions match these filters.</span></div>'; var bulk=document.getElementById('ledgerBulk'); if(bulk) bulk.innerHTML=ledgerBulkHTML(); if(window.lucide)lucide.createIcons(); }
function filterLedger(){ var f=ledgerState.filters, oldMode=f.dateMode; var q=document.getElementById('ledSearch'), cat=document.getElementById('ledCat'), type=document.getElementById('ledType'), dm=document.getElementById('ledDateMode'); if(q)f.q=q.value||''; if(cat)f.cat=cat.value||''; if(type)f.type=(type.value||'').toLowerCase(); if(dm)f.dateMode=dm.value||'all'; ['Date','From','To','Month'].forEach(function(k){var el=document.getElementById('led'+k); if(el) f[k.toLowerCase()]=el.value||'';}); clearLedgerDateValuesForMode(f); if(oldMode!==f.dateMode){ ledgerState.filtersOpen=true; renderPage('ledger'); return; } refreshLedgerRows(); }
function clearLedgerFilters(){ ledgerState.filters={q:'',cat:'',type:'',dateMode:'all',date:'',from:'',to:'',month:''}; ledgerState.filtersOpen=false; renderPage('ledger'); }
function clearLedgerFilter(kind){ var f=ledgerState.filters; if(kind==='q')f.q=''; if(kind==='cat')f.cat=''; if(kind==='type')f.type=''; if(kind==='date'){f.dateMode='all';f.date='';f.from='';f.to='';f.month='';} renderPage('ledger'); }
function toggleLedgerFilters(){ ledgerState.filtersOpen=!ledgerState.filtersOpen; var el=document.getElementById('ledgerFilters'); if(el) el.classList.toggle('open', ledgerState.filtersOpen); }
...
function renderBudgets() {
  ...
  var cards = '<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">' +
    '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Monthly income</div><div class="font-bold mono">' + fmtMoney(cf.total_income)+'</div></div>'+
    '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Monthly expenses</div><div class="font-bold mono">' + fmtMoney(cf.total_expenses)+'</div></div>'+
    ...
    '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Budget remaining</div><div class="font-bold mono">' + fmtMoney(totalRemaining)+'</div></div>'+
    '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Projected result</div><div class="font-bold mono">' + fmtMoney(cf.projected_month_end_surplus_or_deficit)+'</div></div>'+
    '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Investment contributions</div><div class="font-bold mono">' + fmtMoney(cf.investment_contributions)+'</div></div>'+
    '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Debt payments</div><div class="font-bold mono">' + fmtMoney(cf.debt_payments)+'</div></div>'+
    '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Safe to spend/day</div><div class="font-bold mono">' + fmtMoney(M.budgets.reduce(function(s,b){return s+(b.safe||0);},0))+'</div></div></div>';
  var budgetRows = M.budgets.map(function (b) { var rem = b.remaining ?? (b.budget-b.actual); var pctUsed = b.budget > 0 ? Math.min((b.actual / b.budget) * 100, 100) : 0; return '<tr><td class="p-2">'+esc(b.cat||'Total')+'</td><td class="p-2 text-right mono">' + fmtMoney(b.budget)+'</td><td class="p-2 text-right mono">' + fmtMoney(b.actual)+'</td><td class="p-2 text-right mono">' + fmtMoney(rem)+'</td><td class="p-2"><div style="min-width:120px;height:8px;background:var(--c-bg);border-radius:4px"><div style="height:8px;width:'+pctUsed+'%;background:var(--c-focus);border-radius:4px"></div></div></td><td class="p-2">'+esc(b.status||'')+'</td><td class="p-2 text-right"><button class="btn btn-xs" onclick="deleteBudget('+b.id+')">Archive</button></td></tr>'; }).join('');
  var catRows = (M.cashFlowCategories||[]).map(function(c){return '<tr><td class="p-2">'+esc(c.category_name)+'</td><td class="p-2 text-right mono">' + fmtMoney(c.current_month_spending)+'</td><td class="p-2 text-right mono">' + fmtMoney(c.previous_month_spending)+'</td><td class="p-2 text-right mono">'+(c.change_percentage==null?'—':pct(c.change_percentage))+'</td><td class="p-2 text-right mono">' + fmtMoney(c.three_month_average)+'</td><td class="p-2 text-right mono">'+(c.budget_used_percentage==null?'—':pct(c.budget_used_percentage))+'</td></tr>';}).join('');
  ...
  var rec = (M.recurringCandidates||[]).map(function(r){return '<tr><td class="p-2">'+esc(r.merchant)+'</td><td class="p-2">'+esc(r.frequency)+'</td><td class="p-2 text-right mono">' + fmtMoney(r.typical_amount)+'</td><td class="p-2">'+esc(r.next_expected_date)+'</td><td class="p-2 text-right">'+Math.round((r.confidence||0)*100)+'%</td></tr>';}).join('');
...
function renderLiabilities() {
  ...
  var rows = (M.liabilities || []).map(function (l) {
    ...
    return '<div class="card p-4 mb-3">' +
      '<div class="flex items-start justify-between gap-3"><div><div class="font-semibold">' + esc(l.name) + '</div><div class="text-xs" style="color:var(--c-sub);">' + esc(l.institution || l.liability_type) + ' · ' + esc(l.due_status || 'no_due_date') + '</div></div><div class="text-right"><div class="mono font-bold">' + fmtMoney(l.current_outstanding) + '</div><div class="text-[10px]" style="color:var(--c-sub);">' + esc(l.auto_calculation_mode) + '</div></div></div>' +
      '<div class="grid dashboard-two-col grid-cols-2 md:grid-cols-4 gap-2 mt-3 text-xs"><div>EMI<br><span class="mono">' + fmtMoney(l.emi_amount || l.minimum_due) + '</span></div><div>Rate<br><span class="mono">' + (l.interest_rate || 0) + '%</span></div><div>Due<br><span class="mono">' + esc(l.next_due_date || l.due_date || '—') + '</span></div><div>' + (cc ? 'Utilization' : 'Progress') + '<br><span class="mono">' + (cc && l.utilization_percentage != null ? l.utilization_percentage.toFixed(1) + '%' : (l.original_principal ? ((l.original_principal - l.current_outstanding) / l.original_principal * 100).toFixed(1) + '%' : '—')) + '</span></div></div>' +
  ...
  return '<h1 class="text-2xl font-bold" style="color: var(--c-primary);">Liabilities</h1><p class="page-subtitle">Loans, credit cards, EMI tracking, payments, and balance reconciliation.</p>' +
    '<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5"><div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Total outstanding</div><div class="font-bold mono">' + fmtMoney(s.total_outstanding) + '</div></div><div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Monthly EMI</div><div class="font-bold mono">' + fmtMoney(s.monthly_emi_commitment) + '</div></div><div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Overdue</div><div class="font-bold mono">' + fmtMoney(s.overdue_amount) + '</div></div><div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Active</div><div class="font-bold mono">' + fmt(s.active_liability_count) + '</div></div></div>' +
...
async function showLiabilitySchedule(id){ try{ var r=await api('/api/liabilities/'+id+'/schedule'); openModal('Estimated Schedule','<div class="text-xs mb-2" style="color:var(--c-sub);">Calculated schedules are estimates only.</div><div style="overflow-x:auto;max-height:60vh;"><table class="w-full text-xs"><tbody>'+r.schedule.slice(0,120).map(function(x){return '<tr><td class="p-1">'+esc(x.due_date)+'</td><td class="p-1 text-right mono">' + fmtMoney(x.payment_amount)+'</td><td class="p-1 text-right mono">' + fmtMoney(x.principal)+'</td><td class="p-1 text-right mono">' + fmtMoney(x.interest)+'</td><td class="p-1 text-right mono">' + fmtMoney(x.remaining)+'</td></tr>';}).join('')+'</tbody></table></div>'); }catch(e){ toast(e.message,true); } }
...
function renderRecurring() {
  var rows = M.recurring.map(function (r) {
    ...
    return '<div class="card-row" style="display:flex;align-items:center;justify-content:space-between;gap:12px;">' +
      ...
      '</div>' +
      '<div class="text-right flex-shrink-0"><div class="mono text-sm font-semibold">' + fmtMoney(r.amount) + '</div>' +
...
function showAddRecurring() {
  ...
  openModal("Add Recurring", '<div style="display:flex;flex-direction:column;gap:12px;">' +
    ...
    '<div class="grid grid-cols-2 gap-3">' + fld("rcAmount", "Amount (INR)", inp("number")) + fld("rcFreq", "Frequency", sel(freqOpts)) + '</div>' +
...
function renderNetWorth() {
  ...
  function moneyCell(v){ return '<span class="mono">' + fmtMoney(v || 0) + '</span>'; }
  ...
  return '<h1 class="text-2xl font-bold" style="color: var(--c-primary);">Net Worth</h1><p class="page-subtitle">Monthly snapshots, trends, valuation health, and month-end reporting.</p>' +
    ...
    '<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">' +
      '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Current net worth</div><div class="font-bold mono">' + fmtMoney(nw) + '</div></div>' +
      '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Monthly change</div><div class="font-bold mono">' + (mom == null ? '—' : (mom >= 0 ? '+' : '−') + fmtMoney(Math.abs(mom))) + '</div></div>' +
      '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">YTD change</div><div class="font-bold mono">' + (ytd == null ? '—' : (ytd >= 0 ? '+' : '−') + fmtMoney(Math.abs(ytd))) + '</div></div>' +
      ...
      '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Total assets</div><div class="font-bold mono">' + fmtMoney(last.assets) + '</div></div>' +
      '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Investments</div><div class="font-bold mono">' + fmtMoney(last.investments_total || 0) + '</div></div>' +
      '<div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Liabilities</div><div class="font-bold mono">' + fmtMoney(last.liabilities) + '</div></div>' +
...
function updateScenario() {
  ...
  document.getElementById("scenarioCards").innerHTML =
    '<div class="scenario-card p-4 text-center min-w-0"><div class="text-[10px] uppercase mb-1" style="color: var(--c-sub);">New Income</div><div class="mono font-bold break-words ' + incColor + '">' + fmtMoney(Math.round(newInc)) + '</div></div>' +
    '<div class="scenario-card p-4 text-center min-w-0"><div class="text-[10px] uppercase mb-1" style="color: var(--c-sub);">New Expense</div><div class="mono font-bold break-words ' + expColor + '">' + fmtMoney(Math.round(newExp)) + '</div></div>' +
  ...
  document.getElementById("scenarioDetail").innerHTML =
    ...
    '<tr class="border-b" style="border-color: rgba(53,88,114,0.04);"><td class="p-2">Income</td><td class="p-2 text-right mono">' + fmtMoney(M.income) + '</td><td class="p-2 text-right mono ' + incColor + '">' + fmtMoney(Math.round(newInc)) + '</td><td class="p-2 text-right mono">' + (incPct >= 0 ? "+" : "") + incPct + "%</td></tr>" +
    '<tr class="border-b" style="border-color: rgba(53,88,114,0.04);"><td class="p-2">Expense</td><td class="p-2 text-right mono">' + fmtMoney(M.expense) + '</td><td class="p-2 text-right mono ' + expColor + '">' + fmtMoney(Math.round(newExp)) + '</td><td class="p-2 text-right mono">' + (expPct >= 0 ? "+" : "") + expPct + "%</td></tr>" +
    '<tr class="border-b" style="border-color: rgba(53,88,114,0.04);"><td class="p-2">Net Balance</td><td class="p-2 text-right mono">' + fmtMoney(M.net) + '</td><td class="p-2 text-right mono ' + netColor + '">' + fmtMoney(Math.abs(Math.round(newNet))) + '</td><td class="p-2 text-right mono ' + (netDelta >= 0 ? "text-[#4A8C6F]" : "text-[#C44B4B]") + '">' + (netDelta >= 0 ? "+" : "") + fmt(netDelta) + "</td></tr>" +
...
function renderCreditCard() {
  var cards = M.creditCards.map(function (c, idx) {
    ...
    return '<div class="card p-5 mb-3"><div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px;min-height:40px;"><div><h3 class="font-semibold">' + esc(c.name) + ' <button onclick="showEditCreditCard(' + idx + ')" style="background:none;border:none;cursor:pointer;color:var(--c-sub);font-size:11px;margin-left:4px;"><i data-lucide="pencil" class="w-3.5 h-3.5"></i></button></h3><p class="text-xs" style="color: var(--c-sub);">Limit: ' + fmtMoney(c.limit) + " · Statement: " + ordinal(c.statementDay) + " · Due: " + ordinal(c.dueDay) + '</p></div><div class="text-right flex-shrink-0"><div class="mono text-lg font-bold" style="color: var(--c-ink);">' + fmtMoney(c.balance) + '</div><div class="text-xs" style="color:' + utilColor + ';">' + utilization + "% utilized</div></div></div>" +
      ...
      '<div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs"><div><span style="color:var(--c-sub);">Min Payment</span><br><span class="mono">' + fmtMoney(minPay) + '</span></div><div><span style="color:var(--c-sub);">Interest</span><br><span class="mono">' + c.interestRate + '%/mo</span></div><div><span style="color:var(--c-sub);">Annual Fee</span><br><span class="mono">' + fmtMoney(c.annualFee) + '</span></div><div><span style="color:var(--c-sub);">Available</span><br><span class="mono text-[#4A8C6F]">' + fmtMoney(c.limit - c.balance) + "</span></div></div></div>";
  }).join("");
  ...
  return '<h1 class="text-2xl font-bold" style="color: var(--c-primary);">Credit Card</h1><p class="page-subtitle">Track credit card balances, utilization, and billing cycles</p>' +
    ...
    '<div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4"><div class="card p-3 text-center min-w-0"><div class="text-[10px] text-[#6B7D8E] uppercase">Total Balance</div><div class="mono font-bold text-[#C44B4B] break-words">' + fmtMoney(totalBalance) + '</div></div><div class="card p-3 text-center min-w-0"><div class="text-[10px] text-[#6B7D8E] uppercase">Total Limit</div><div class="mono font-bold break-words">' + fmtMoney(totalLimit) + '</div></div><div class="card p-3 text-center min-w-0"><div class="text-[10px] text-[#6B7D8E] uppercase">Utilization</div><div class="mono font-bold">' + (totalLimit > 0 ? (totalBalance / totalLimit * 100).toFixed(0) : 0) + "%</div></div></div>" +
...
function renderAssets() {
  var depRows = M.deposits.map(function (d, idx) {
    ...
    return '<div class="card p-4 mb-2"><div style="display:flex;align-items:center;justify-content:space-between;"><div><span class="text-sm font-medium">' + esc(d.bank) + ' <button onclick="showEditDeposit(' + idx + ')" style="background:none;border:none;cursor:pointer;color:var(--c-sub);font-size:11px;margin-left:4px;"><i data-lucide="pencil" class="w-3.5 h-3.5"></i></button></span><span class="text-xs ml-2" style="color: var(--c-sub);">' + d.rate + "% p.a. · " + d.tenor + 'mo</span></div><div class="text-right"><span class="mono text-sm font-bold">' + fmtMoney(d.amount) + '</span><span class="ml-2 text-xs px-1.5 py-0.5 rounded ' + (isActive ? "text-[#4A8C6F] bg-[rgba(74,140,111,0.08)]" : "text-[#6B7D8E] bg-[rgba(107,125,142,0.08)]") + '">' + (isActive ? "Active" : "Matured") + '</span></div></div><div class="grid grid-cols-3 gap-2 mt-2 text-xs"><div><span style="color:var(--c-sub);">Interest</span><br><span class="mono text-[#4A8C6F]">' + fmtMoney(Math.round(interest)) + '</span></div><div><span style="color:var(--c-sub);">Maturity Value</span><br><span class="mono">' + fmtMoney(Math.round(matVal)) + '</span></div><div><span style="color:var(--c-sub);">' + (isActive ? "Matures" : "Matured") + "</span><br><span class=\"mono\">" + (d.maturity || "").slice(0, 10) + "</span></div></div></div>";
  }).join("");
  ...
  return '<h1 class="text-2xl font-bold" style="color: var(--c-primary);">Assets</h1><p class="page-subtitle">Track deposits and total asset value — update monthly</p>' +
    '<div class="card p-5 mb-4"><div class="flex items-center justify-between mb-3"><div class="section-title" style="margin-bottom:0;">Deposits</div><button class="btn-primary px-3 py-1.5 rounded-lg text-xs flex items-center gap-1" onclick="showAddDeposit()"><i data-lucide="plus" class="w-3.5 h-3.5"></i> Add</button></div>' + (depRows || '<div class="text-xs" style="color:var(--c-sub);">No deposits</div>') + '<div class="text-right text-xs mt-2 mono" style="color: var(--c-sub);">Total Deposits: ' + fmtMoney(totalDep) + "</div></div>" +
    '<div class="card p-5"><div class="flex items-center justify-between mb-3"><div class="section-title" style="margin-bottom:0;">Investment Accounts</div><button class="btn-primary px-3 py-1.5 rounded-lg text-xs flex items-center gap-1" onclick="showAddPortfolio()"><i data-lucide="plus" class="w-3.5 h-3.5"></i> Add</button></div>' + (portoCards || '<div class="text-xs" style="color:var(--c-sub);">No portfolios</div>') + '<div class="text-right text-xs mt-2 mono" style="color: var(--c-sub);">Total Portfolio: ' + fmtMoney(totalPorto) + "</div></div>";
}
...
function renderWallets() {
  ...
  var walletCards = M.wallets.map(function (w, idx) {
    ...
    return '<div class="card p-4 mb-3" onclick="toggleWalletDetail(this)" style="cursor:pointer;">' +
      ...
      '<div class="text-right"><div class="mono text-base font-bold ' + (isNeg ? "text-[#C44B4B]" : "text-[#4A8C6F]") + '">' + fmtMoney(Math.abs(bal)) + '</div><div class="text-[10px]" style="color:var(--c-sub);">' + (isNeg ? "deficit" : "available") + '</div></div><button class="flex-shrink-0 p-2" style="color:var(--c-sub);background:none;border:none;cursor:pointer;" onclick="event.stopPropagation();showWalletActions(' + idx + ')" aria-label="Wallet actions"><i data-lucide="ellipsis-vertical" class="w-4 h-4"></i></button></div>' +
      '<div class="wallet-detail" style="display:none;margin-top:16px;padding-top:12px;border-top:1px solid var(--c-border);"><div class="text-[10px] font-semibold uppercase mb-2" style="color:var(--c-sub);letter-spacing:0.05em;">Reconciliation</div><div class="grid grid-cols-2 gap-2 mb-3"><div class="flex justify-between text-[11px] px-2 py-1 rounded" style="background:rgba(74,140,111,0.04);"><span style="color:var(--c-sub);">Income</span><span class="mono text-[#4A8C6F]">+' + fmtMoney(bd.income || 0) + '</span></div><div class="flex justify-between text-[11px] px-2 py-1 rounded" style="background:rgba(74,140,111,0.04);"><span style="color:var(--c-sub);">Transfers In</span><span class="mono text-[#4A8C6F]">+' + fmtMoney(bd.transfersIn || 0) + '</span></div><div class="flex justify-between text-[11px] px-2 py-1 rounded" style="background:rgba(196,75,75,0.04);"><span style="color:var(--c-sub);">Expenses</span><span class="mono text-[#C44B4B]">−' + fmtMoney(bd.expenses || 0) + '</span></div><div class="flex justify-between text-[11px] px-2 py-1 rounded" style="background:rgba(196,75,75,0.04);"><span style="color:var(--c-sub);">Transfers Out</span><span class="mono text-[#C44B4B]">−' + fmtMoney(bd.transfersOut || 0) + '</span></div></div><div class="flex justify-between text-xs font-semibold px-2 py-2 rounded mb-3" style="background:rgba(53,88,114,0.04);"><span>Net Balance</span><span class="mono ' + (bal >= 0 ? "text-[#4A8C6F]" : "text-[#C44B4B]") + '">' + fmtMoney(Math.abs(bal)) + '</span></div><div class="text-[10px] font-semibold uppercase mb-2" style="color:var(--c-sub);letter-spacing:0.05em;">Activity</div>' + (txnRows || '<div class="text-xs" style="color:var(--c-sub);">No transactions yet</div>') + "</div></div>";
  }).join("");
  return '<h1 class="text-2xl font-bold" style="color:var(--c-primary);">Wallets</h1><p class="page-subtitle">Track balances — income, expenses, and transfers — all reconcilable</p>' +
    '<div class="card p-4 mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><div class="text-[10px] uppercase" style="color:var(--c-sub);">Total Liquid</div><div class="mono text-lg font-bold ' + (totalLiquid >= 0 ? "text-[#4A8C6F]" : "text-[#C44B4B]") + '">' + fmtMoney(Math.abs(totalLiquid)) + '</div></div><div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto"><button class="btn-primary px-4 py-2 rounded-lg text-sm flex items-center justify-center gap-1.5" onclick="showAddWallet()"><i data-lucide="plus" class="w-4 h-4"></i> Add Wallet</button><button class="btn-primary px-4 py-2 rounded-lg text-sm flex items-center justify-center gap-1.5" onclick="showTransfer()"><i data-lucide="arrow-left-right" class="w-4 h-4"></i> Transfer</button></div></div>' +
...
function renderPennyWise() {
  var s = M._pennywise || { enabled: true, counts: [], clients: [], recent_errors: [] };
  var dup = M._pennywiseDup || { candidates: [] };
  var counts = {}; (s.counts || []).forEach(function (r) { counts[r.sync_status] = r.count; });
  var clientRows = (s.clients || []).map(function (c) { return '<tr><td class="p-2 mono text-xs">' + esc(c.client_id) + '</td><td class="p-2 text-xs">' + esc(c.last_seen_at || '—') + '</td><td class="p-2 text-right mono">' + fmt(c.records) + '</td></tr>'; }).join('');
  var errorRows = (s.recent_errors || []).map(function (e) { return '<div class="card-row"><span class="mono text-xs">' + esc(e.client_transaction_id) + '</span><span class="text-xs flex-1" style="color:var(--c-danger);">' + esc(e.error_code || '') + ' ' + esc(e.error_message || '') + '</span><span class="text-[10px]" style="color:var(--c-sub);">' + esc(e.updated_at || '') + '</span></div>'; }).join('');
  var dupRows = (dup.candidates || []).map(function (g) {
    var movements = (g.involved_movements || []).map(function (m) {
      return '<div class="rounded p-2 text-xs" style="background:rgba(53,88,114,.04);"><b>#' + esc(m.movement_id) + '</b> ' + esc(m.date || '') + ' ' + esc(m.time || '') + ' · ' + fmtMoney(m.amount) + ' · ' + esc(m.wallet_name || m.wallet_id || '') + '<br>' + esc(m.merchant || m.description || '') + (m.reference_number ? ' · <span class="mono">' + esc(m.reference_number) + '</span>' : '') + '<br><a class="underline" href="/?page=ledger&q=' + encodeURIComponent(m.reference_number || m.merchant || '') + '" data-page="ledger">Open movement</a></div>';
    }).join('');
    var syncs = (g.involved_sync_records || []).map(function (r) { return '<div class="text-[10px] mono">sync #' + esc(r.sync_record_id) + ' · client ' + esc(r.client_id || '') + ' · tx ' + esc(r.client_transaction_id || '') + ' · sms ' + esc(r.sms_fingerprint || r.raw_sms_hash || '—') + '</div>'; }).join('');
    var ids = (g.involved_movements || []).map(function (m) { return m.movement_id; });
    var primary = ids[0], duplicate = ids[1];
    var duplicateMovement = (g.involved_movements || []).find(function (m) { return m.movement_id === duplicate; }) || {};
    return '<section class="card p-4 space-y-3"><div class="flex flex-col md:flex-row md:items-start md:justify-between gap-2"><div><div class="font-bold">' + esc(g.classification || 'duplicate candidate') + ' · ' + esc(g.confidence) + '</div><p class="text-xs" style="color:var(--c-sub);">' + esc(g.reason || '') + '</p></div><div class="mono text-sm">' + fmtMoney(g.current_ledger_impact || 0) + '</div></div><div class="grid md:grid-cols-2 gap-2">' + movements + '</div><div class="text-xs"><b>Matching:</b> ' + esc((g.matching_fields || []).join(', ') || '—') + '<br><b>Differing:</b> ' + esc((g.differing_fields || []).join(', ') || '—') + '</div>' + syncs + '<div class="flex flex-wrap gap-2"><button class="btn btn-xs" onclick="reviewPennyWiseDuplicate(\'' + esc(g.candidate_group_id) + '\',\'keep_all\')">Keep all</button><button class="btn btn-xs" onclick="reviewPennyWiseDuplicate(\'' + esc(g.candidate_group_id) + '\',\'defer\')">Defer review</button><button class="btn btn-xs" onclick="reviewPennyWiseDuplicate(\'' + esc(g.candidate_group_id) + '\',\'ignore\')">Ignore candidate</button>' + (primary && duplicate ? '<button class="btn btn-warning btn-xs" onclick="markPennyWiseDuplicate(\'' + esc(g.candidate_group_id) + '\',' + primary + ',' + duplicate + ',' + Number(duplicateMovement.amount || 0) + ')">Mark #' + duplicate + ' duplicate of #' + primary + '</button>' : '') + '</div></section>';
  }).join('');
  return '<h1 class="text-2xl font-bold" style="color: var(--c-primary);">PennyWise Integration</h1><p class="page-subtitle">Reviewable SMS sync from PennyWise Android. API tokens are managed on API & AI and are never shown here.</p>' +
    '<div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5"><div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Enabled</div><div class="font-bold">' + (s.enabled ? 'Yes' : 'No') + '</div></div><div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Received</div><div class="font-bold mono">' + fmt(Object.keys(counts).reduce(function(a,k){return a+Number(counts[k]||0);},0)) + '</div></div><div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Created</div><div class="font-bold mono">' + fmt(counts.created || counts.already_synced || 0) + '</div></div><div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Duplicates</div><div class="font-bold mono">' + fmt(counts.possible_duplicate || 0) + '</div></div><div class="card p-4"><div class="text-[10px] uppercase" style="color:var(--c-sub)">Failed</div><div class="font-bold mono">' + fmt(counts.validation_failed || counts.mapping_missing || counts.server_error || 0) + '</div></div></div>' +
    '<div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4"><div class="card p-5"><div class="section-title">Connected client IDs</div><table class="w-full text-sm"><tbody>' + (clientRows || '<tr><td class="p-2 text-xs" style="color:var(--c-sub);">No clients yet</td></tr>') + '</tbody></table></div><div class="card p-5"><div class="section-title">Recent sync errors</div>' + (errorRows || '<div class="text-xs" style="color:var(--c-sub);">No recent errors</div>') + '</div></div><section class="space-y-3"><div class="flex items-center justify-between gap-2"><div><h2 class="section-title">Duplicate Review</h2><p class="text-xs" style="color:var(--c-sub);">Settings → Integrations → PennyWise → Duplicate Review. Reviews preserve movements and sync records; marking a duplicate only soft-excludes it after dependency checks.</p></div><button class="btn btn-sm" onclick="loadPennyWiseSummary()">Refresh</button></div>' + (dupRows || '<div class="card p-4 text-xs" style="color:var(--c-sub);">No unresolved duplicate candidates.</div>') + '</section>';
}
async function loadPennyWiseSummary() { try { M._pennywise = await api('/api/integrations/pennywise/summary'); M._pennywiseDup = await api('/api/integrations/pennywise/duplicate-candidates?unresolved_only=true'); M._pennywiseLoaded = true; var pc = document.getElementById('pageContent'); if (currentPage === 'pennywise' && pc) { pc.innerHTML = renderPennyWise(); if (window.lucide) lucide.createIcons(); } } catch(e) { toast(e.message, true); } }
async function reviewPennyWiseDuplicate(groupId, action) { try { await api('/api/integrations/pennywise/duplicate-candidates/review', { method: 'POST', body: { candidate_group_id: groupId, action: action } }); await loadPennyWiseSummary(); toast('Review saved'); } catch(e) { toast(e.message, true); } }
async function markPennyWiseDuplicate(groupId, retainedId, duplicateId, impact) { var msg = 'Mark movement #' + duplicateId + ' as duplicate of #' + retainedId + '?\n\nBalance impact: ' + fmtMoney(impact) + '\nCash-flow impact: ' + fmtMoney(impact) + '\n\nThe duplicate row and PennyWise sync history will be preserved. If locked reconciliations, locked snapshots, or downstream links exist, the API will block this action.'; if(!confirm(msg)) return; try { await api('/api/integrations/pennywise/duplicate-candidates/review', { method: 'POST', body: { candidate_group_id: groupId, action: 'mark_duplicate', retained_movement_id: retainedId, duplicate_movement_id: duplicateId, confirm: true } }); await reload('pennywise'); toast('Duplicate soft-excluded'); } catch(e) { toast(e.message, true); } }
...
function renderAPI() {
  var endpoints = [
    ...
    ["GET", "/api/integrations/pennywise/duplicate-candidates", "Review suspected historical PennyWise Ledger duplicates without changing data"],
  ];
...
function showEditDeposit(idx) {
  ...
  openModal("Edit Deposit at " + d.bank, '<div style="display:flex;flex-direction:column;gap:12px;">' +
    fld("edAmount", "Amount (INR)", inp("number"), d.amount) +
...
function showTransfer() {
  ...
  openModal("Transfer", '<div style="display:flex;flex-direction:column;gap:12px;"><div class="grid grid-cols-2 gap-3">' + fld("tfFrom", "From", sel(opts)) + fld("tfTo", "To", sel(opts)) + "</div>" + fld("tfAmount", "Amount (INR)", inp("number")) + fld("tfNotes", "Notes", inp("text")) + saveBtn("Transfer", "doTransfer()") + "</div>");
}
...
async function doWalletExpense(id) {
  ...
  try { var r = await api("/api/wallets/" + id + "/expense", { method: "POST", body: { amount: amt, category_id: catId(document.getElementById("weCat").value), date: document.getElementById("weDate").value, description: document.getElementById("weDesc").value } }); closeModal(); if (r.warning) { await reload("wallets"); alert("Warning: Spending " + fmtMoney(r.warning.amount) + " exceeds free balance (" + fmtMoney(r.warning.free) + "). " + fmtMoney(r.warning.into) + " will come from earmarked goals: " + (r.warning.impactedGoals.join(", ") || "none")); } else { await reload("wallets"); toast("Saved"); } } catch (e) { toast(e.message, true); }
}
...
function showAddEarmark(goalIdx) {
  ...
  openModal("Allocate to " + g.name, '<div style="display:flex;flex-direction:column;gap:12px;">' + fld("aeSource", "Source", sel(sources.map(function (s) { return "<option>" + esc(s) + "</option>"; }).join(""))) + fld("aeAmt", "Amount (INR)", inp("number")) + saveBtn("Allocate", "doAddEarmark(" + g.id + ")") + "</div>");
}
...
function renderReconList(rows){ return '<div class="space-y-2">'+(rows.map(function(r){return '<div class="card p-3"><div class="flex flex-col sm:flex-row sm:justify-between gap-2"><div><b>'+esc(r.wallet_name||r.wallet_id)+'</b><div class="text-xs text-[var(--c-sub)]">'+r.period_start+' → '+r.period_end+' • '+r.status+(r.locked?' • locked':'')+'</div></div><div class="font-mono">Diff '+fmtMoney(r.difference||0)+'</div></div><div class="grid dashboard-two-col grid-cols-2 md:grid-cols-4 gap-2 text-sm mt-3"><div>Opening<br><b>'+fmtMoney(r.opening_balance||0)+'</b></div><div>Statement<br><b>'+fmtMoney(r.statement_closing_balance||0)+'</b></div><div>Calculated<br><b>'+fmtMoney(r.expected_closing_balance||0)+'</b></div><div>Matched<br><b>'+(r.matched_count||0)+'</b></div></div><div class="mt-3 flex gap-2 flex-wrap"><button class="btn" onclick="previewRecon('+r.id+')">Preview</button><button class="btn" onclick="autoMatchRecon('+r.id+')">Auto-match</button><button class="btn" onclick="lockRecon('+r.id+')">Lock</button></div></div>';}).join('')||'<p class="text-sm text-[var(--c-sub)]">No reconciliations.</p>')+'</div>'; }
