(function(g){
  ...
  function keyExample(type){return ({stock:'stock:HINDALCO',mutual_fund:'mf:122639',crypto:'crypto:BTC-INR',other:'ulip:hdfc-opportunities-fund'}[type]||'stock:HINDALCO');}
  function marketSettings(a){return '<div class="mt-3 p-3 rounded border"><div class="text-xs font-semibold mb-2">Google Sheets price feed</div>'+H.field('as_price_feed_asset_key','Google Sheets asset key','text',a.price_feed_asset_key)+'<p class="text-xs" style="color:var(--c-sub);">Example: '+H.esc(keyExample(a.asset_type||'stock'))+'</p><p class="text-xs" style="color:var(--c-sub);">Mapping status: '+H.esc(a.price_feed_asset_key?'Mapped':'Not mapped')+'</p><label class="wealth-check"><input id="as_auto_refresh" type="checkbox" '+(a.automatic_price_refresh?'checked':'')+'> Include in automated refresh</label><p class="text-xs" style="color:var(--c-sub);">Last price date: '+H.esc(a.last_provider_trade_date||'—')+'</p></div><div class="mt-3 p-3 rounded border"><div class="text-xs font-semibold mb-2">Legacy direct-provider settings — currently inactive</div>'+H.select('as_price_provider','Provider',[{value:'manual',label:'Manual'},{value:'nse_bhavcopy',label:'NSE official close'},{value:'yahoo_finance',label:'Yahoo fallback'},{value:'mfapi',label:'MFAPI mutual-fund NAV'}],a.price_provider||'manual')+H.field('as_provider_symbol','Provider symbol','text',a.provider_symbol)+H.field('as_provider_exchange','Provider exchange','text',a.provider_exchange)+H.field('as_provider_scheme_code','Provider scheme code','text',a.provider_scheme_code)+'<p class="text-xs" style="color:var(--c-sub);">Last legacy source: '+providerLabel(a.price_provider)+'</p></div>'; }
  ...
  async function save(id){
    try{
      ...
      var b={name:H.val('as_name'),asset_type:t,symbol:H.val('as_symbol'),isin:H.val('as_isin'),exchange:H.val('as_exchange'),scheme_code:H.val('as_scheme_code'),currency:H.val('as_currency')||'INR',price_source:H.val('as_src'),pricing_mode:H.val('as_mode'),valuation_mode:H.val('as_val_mode')||null,is_active:H.val('as_active'),notes:H.val('as_notes'),metadata:meta,price_feed_asset_key:H.val('as_price_feed_asset_key'),price_provider:H.val('as_price_provider'),provider_symbol:H.val('as_provider_symbol'),provider_exchange:H.val('as_provider_exchange'),provider_scheme_code:H.val('as_provider_scheme_code'),automatic_price_refresh:!!document.getElementById('as_auto_refresh')?.checked};
